...
def setup_directories():
    directories = ['logs', 'backups', 'informes', 'pedidos']
    for directory in directories:
        if not os.path.exists(directory):
            os.makedirs(directory)
    # Configurar el entorno antes de iniciar
        try:
            # Crear directorios necesarios
            setup_directories()
            # Configurar logging
...
def main():
    try:
        # Verificar sistema y crear directorios necesarios
        setup_environment()
...
def setup_environment():
    ...
    try:
        # Crear directorios necesarios
        dirs = ['backups', 'informes', 'pedidos']
        for dir in dirs:
            if not os.path.exists(dir):
                os.makedirs(dir)
        
        # Verificar base de datos
        if not os.path.exists('heladeria.db'):
            create_database()
            
    except Exception as e:
        raise Exception(f"Error al configurar el entorno: {str(e)}")
